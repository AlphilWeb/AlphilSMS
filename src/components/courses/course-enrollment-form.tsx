...
import { enrollInCourse } from '@/lib/actions/student.course.actions'; // Changed import
...
interface Course {
  ...
  name: string;
  code: string;
  credits: number; // Changed from string to number to match the action return type
...
export default function CourseEnrollmentForm({ availableCourses }: CourseEnrollmentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Changed to use enrollInCourse action instead
                  ...
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-gray-500">
                        ...
                        {course.lecturer && (
                          <span> • Lecturer: {course.lecturer.firstName} {course.lecturer.lastName}</span>
                        )}
                      </p>
                    </div>
                    <input
