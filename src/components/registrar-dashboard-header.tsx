...
import { getRegistrarHeaderData, logout, RegistrarHeaderData } from '@/lib/actions/registrar.dashboard.header';

export default function RegistrarDashboardHeader() {
  const pathname = usePathname();
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [registrarData, setRegistrarData] = useState<RegistrarHeaderData | null>(null);
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        const data = await getRegistrarHeaderData();
  ...
  const navItems = [
    ...
    },
    {
      category: "Account",
      items: [
        { href: "/dashboard/registrar/profile", icon: <FiUser className="w-5 h-5" />, label: "Profile" },
      ]
    }
  ];
        ...
        <div className="px-6 py-1 flex items-center justify-between">
          ...
          <div className="flex items-center space-x-6">
            <button className="relative p-1 rounded-full text-gray-600 hover:text-gray-900 hover:bg-gray-100">
              {registrarData.notificationCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-pink-500 text-white text-xs font-bold rounded-full h-5 w-5 flex items-center justify-center">
                  {registrarData.notificationCount}
                </span>
              )}
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden md:block text-left">
                  <p className="text-sm font-medium text-gray-800">{registrarData.name}</p>
                  <p className="text-xs text-gray-500">{registrarData.role}</p>
                </div>
              ...
              {isDropdownOpen && (
                <div className="absolute right-0 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-50">
                  <div className="px-4 py-3">
                    <p className="text-sm font-medium text-gray-900">{registrarData.name}</p>
                    <p className="text-sm text-gray-500 truncate">{registrarData.role.charAt(0).toUpperCase() + registrarData.role.slice(1)}</p>
                    {registrarData.department && (
                      <p className="text-xs text-gray-500 truncate">{registrarData.department}</p>
                    )}
                  </div>
                  <div className="py-1">
                    <Link
                      href="/dashboard/registrar/profile"
